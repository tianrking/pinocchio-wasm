...
impl Model {
    ...
    fn emit_mjcf_body(&self, link_idx: usize, indent: usize, out: &mut String) -> Result<()> {
        ...
        if link_idx != 0 {
            ...
            if joint.jtype != JointType::Fixed {
                out.push_str(&format!(
                    "{pad}  <joint name=\"j{}\" type=\"{}\" axis=\"{}\" pos=\"{}\"/>\n",
                    link_idx,
                    mjcf_type,
    ...
    pub fn from_mjcf_str(input: &str) -> Result<Self> {
        let doc = roxmltree::Document::parse(input)
            .map_err(|_| PinocchioError::invalid_model("failed to parse mjcf xml"))?;
        let mujoco = doc.descendants().find(|n| n.has_tag_name("mujoco")).ok_or(
            PinocchioError::invalid_model("mjcf must contain a <mujoco> root"),
        )?;
...
fn parse_mjcf_body_tree(
    ...
    if link_specs.contains_key(&body_name) {
        return Err(PinocchioError::invalid_model("duplicate body name in mjcf"));
    }
    ...
    if let Some(parent) = parent_name {
        let joint_node = body_node.children().find(|n| n.has_tag_name("joint"));
        let jtype = if let Some(jn) = joint_node {
            let joint_type = jn.attribute("type").unwrap_or("hinge");
            match joint_type {
            ...
            // No joint node means fixed attachment
            JointType::Fixed
        };

        let axis = if let Some(jn) = joint_node {
            jn.attribute("axis")
                .map(parse_vec3_text)
                .transpose()?
        ...
        let origin = if let Some(jn) = joint_node {
            if let Some(pos) = jn.attribute("pos") {
                parse_vec3_text(pos)?
